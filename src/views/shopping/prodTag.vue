    ...
    >
      ...
      <template slot-scope="scope" slot="menu">
        <el-button type="primary" size="small" icon="el-icon-edit" @click="addOrUpdateHandle(scope.row.id)"
          >修改</el-button
        >
        <el-button type="danger" icon="el-icon-delete" size="small" @click.stop="deleteHandle(scope.row.id)"
          >删除</el-button
        >
      </template>
    </avue-crud>
...
export default {
  ...
  methods: {
    getDataList(page, params, done) {
      ...
      this.$httpShop({
        url: this.$httpShop.adornUrl('/prod/prodTag/page'),
        method: 'get',
    ...
    deleteHandle(id) {
      this.$confirm('确定进行删除操作?', '提示', {
        ...
        .then(() => {
          this.$httpShop({
            url: this.$httpShop.adornUrl('/prod/prodTag/' + id),
            method: 'delete',
