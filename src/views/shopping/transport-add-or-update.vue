      ...
      <el-table :data="dataForm.transfees" border style="width: 100%" class="table-con">
        <el-table-column header-align="center" align="center" width="450" label="可配送区域">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.$index > 0" type="text" size="small" @click="addOrUpdateHandle(`${scope.$index}`)"
              >编辑</el-button
            >
            <el-button v-if="scope.$index > 0" type="text" size="small" @click="deleteHandle(`${scope.$index}`)"
              >删除</el-button
            >
          </template>
        </el-table-column>
      ...
      >
        <el-table-column header-align="center" align="center" width="350" label="指定区域">
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="addOrUpdateTransfeeFree(`${scope.$index}`)">编辑</el-button>
            <el-button type="text" size="small" @click="deleteTransfeeFree(`${scope.$index}`)">删除</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column header-align="center" align="left">
          <template slot-scope="scope">
            ...
            <el-form-item
              ...
              :rules="[{ required: true, message: `不能为空` }]"
            >
              满 <el-input style="width: 100px" v-model="scope.row.piece"></el-input> 件/重量/体积包邮
            </el-form-item>
          </template>
        </el-table-column>
      </el-table>
      <div style="margin-top: 20px" v-if="dataForm.isFreeFee == 0">
        <el-button type="primary" icon="el-icon-location-outline" @click="addTransfeeFree()"
    ...
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        this.dataForm.transfees = [
          { cityList: [], status: 0, firstPiece: 1, firstFee: 0, continuousPiece: 1, continuousFee: 0 },
        ]
      })
      if (this.dataForm.transportId) {
        this.$httpShop({
          // 获取运费模板数据
          url: this.$httpShop.adornUrl(`/shop/transport/info/${this.dataForm.transportId}`),
          method: 'get',
        }).then(({ result }) => {
          if (result.isFreeFee) {
            result.transfees[0].status = 0
          } else {
            result.transfees[0].status = 1
          }
          this.dataForm = result
          this.dataForm.hasFreeCondition = !!result.hasFreeCondition
          this.dataForm.transfees = [
            { cityList: [], status: 0, firstPiece: 1, firstFee: 0, continuousPiece: 1, continuousFee: 0 },
          ]
        })
