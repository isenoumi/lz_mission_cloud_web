  ...
  <div class="mod-prod-sku-tag">
    <el-form-item label="商品规格">
      ...
      <div v-for="(tag, tagIndex) in skuTags" :key="tagIndex">
        ...
        <el-button class="button-new-tag" type="text" icon="el-icon-delete" @click="removeTag(tagIndex)"
          >删除</el-button
        >
        <br />
        <el-tag
...
export default {
  ...
  created: function () {
    ...
    this.$httpShop({
      url: this.$httpShop.adornUrl(`/prod/spec/listSpecMaxValueId`),
      method: 'get',
      params: this.$httpShop.adornParams(),
    }).then(({ result }) => {
      if (result) {
        this.maxValueId = result
      } else {
        this.maxValueId = 0
      }
    })
  ...
  methods: {
    ...
    addTag() {
      ...
      if (!this.addTagInput.propName) {
        this.$messageE.error('请输入规格名')
        return
      }
      if (!selectValues.length) {
        this.$messageE.error('请输入规格值')
        return
      }
      this.isShowTagInput = false
    ...
    handleTagClick() {
      ...
      this.$httpShop({
        url: this.$httpShop.adornUrl(`/prod/spec/listSpecValue/${this.dbTags[specsIndex].propId}`),
        method: 'get',
        params: this.$httpShop.adornParams(),
      }).then(({ result }) => {
        this.dbTagValues = result
      })
    },
    // 关闭标签 --删除
    ...
    checkTagItem(tagIndex) {
      ...
      this.skuTags.forEach((tag) => {
        ...
        if (arr.indexOf(tagItem) > -1) {
          isSame = true
          this.$messageE.error('product.specificationValue')
          return false
        }
      })
      return isSame
