...
export default {
  ...
  methods: {
    initDictConfig(){
      getAction("/mission/bkbChoiceqst_answer/queryById", { id: this.choiceqstData.id }).then(res => {
        if (res.success) {
          if(!this.regionData){
            this.regionData = new Area(this.$Jpcaa).getText(res.result.region);
          }
          getAction('/mission/bkbChoiceqst_answer/queryBkbChoiceqstAnswerByMainId', { id: this.choiceqstData.id }).then(res => {
            if (res.success) {
          ...
          this.choiceData = { ...res.result, choiceType_dictText: filterDictTextByCache("answer_type", res.result.choiceType), status_dictText: filterDictTextByCache("publish_type_status", res.result.status) };
        }
      });
